...
async def _safe_send(bot: Bot, chat_id: int, text: str) -> None:
    """Send a notification, treating a blocked bot / deactivated chat as an
    expected outcome (logged at info level) rather than an error worth alarming
    on — as opposed to other Telegram API failures, which are real problems."""
    try:
...
async def notify_manager_new_ticket(bot: Bot, manager_telegram_id: int, ticket, user, message_text: str, language: str = "ru"):
    ...
    await _safe_send(bot, manager_telegram_id, notification)


async def notify_manager_new_message(bot: Bot, manager_telegram_id: int, ticket, message_text: str, language: str = "ru"):
